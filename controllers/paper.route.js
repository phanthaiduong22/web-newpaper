...
router.get('/details/:id', async function (req, res) {
  ...
  let comments = await commentModel.findAllCommentByPaperId(paperId);
  let guestComments = await commentModel.findGuestCommentsByPaperId(paperId);
  guestComments = guestComments.map((comment) => {
    return {
      ...comment,
      Name: 'anonymous',
      Role: 'guest',
  ...
  comments = [...comments, ...guestComments];
...
router.post('/details/:id/comment', async function (req, res) {
  const { content } = req.body;
  const userId = req.session.authUser ? req.session.authUser.UserID : 'guest';
  const paperId = +req.params.id || 0;
  const comment = {
    PaperID: paperId,
    UserID: userId,

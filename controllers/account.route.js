...
router.post("/register", notAuth, async function (req, res) {
  ...
  const isValidDate = moment(req.body.raw_dob).isValid();
  const dob = isValidDate
    ? moment(req.body.raw_dob, "DD/MM/YYYY").format("YYYY-MM-DD")
...
router.post("/profile/extend", authUser, async (req, res) => {
  const userId = req?.query.id ? req.query.id : req.session.authUser.UserID;
